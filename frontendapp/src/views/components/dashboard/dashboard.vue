<template>
    <div class="margin-xl">
      <div>
        <button class="btn add" @click="changeViewList" v-if="!showTask">
          {{ showCreateList ? "Retour" : "Ajouter une liste" }}
        </button>
      </div>
        ...
        <div style="margin-left: 20px;" v-if="listes && listes.length > 0">
          <div class="flex" style="flex-wrap: wrap; justify-content: center;">
            <div v-for="list in listes" class="box-liste" :class="'liste'+list.liste.style" @click="openTask(list)">
              <p>{{ list.liste.name }}</p>
              <p v-if="data.user.id != list.liste.owner" style="font-size: 12px; position: absolute; top: 120px;">
                Créé par : {{ list.owner.firstname + " " + list.owner.name }}
              </p>
            </div>
          </div>
        </div>
        <p v-else style="margin-left: 20px;">
          Vous n'avez pas de listes, <a style="color: blue;" @click="changeViewList">Créer une liste</a>.
        </p>
      </div>
